...
function AddGame() {
  ...
  const draft = {
    id: localStorage.getItem('draft_id') || JSON.stringify(Math.random()*1000),
    pictures:images,
    ...draftStored
  }
  const addImage = (e)=>{
  ...
  const handleSubmit = async(e)=>{
      e.preventDefault()
        ...
        console.log({...others})
       axios.post(`/games/game/${user.token}`,{...others}).then(res=>{
    ...
    <div className='admin-container games'>
      <div>
        ...
        <label htmlFor='Name'>
          ...
          <input type='text' onChange={(e)=>updateDraft('name',e.target.value)} placeholder='Example: Tomb Raider' defaultValue={draftStored? draftStored.name : ''} required/>
        </label>
        <label htmlFor='description'>
          <h3>Enter Game Description</h3>
          <textarea maxLength={2000} onChange={(e)=>updateDraft('description',e.target.value)} placeholder='Example: Tomb Raider...............' defaultValue={draftStored? draftStored.description :''} minLength={300}>
          </textarea>
        </label>
        <label htmlFor='Picture'>
              <div className='image-provided'>
                {(images.length > 0) && images.map(image=>(
                  <div className='image-div'><img src={`${image}`} alt='React Gamer App'/><div className='close' ><p onClick={removeImage}>X</p></div></div>
                ))}
              </div>
